...
This validation uses the same 300 held-out test members for the GLMNet T-learner and GLMNet X-learner. These models are the primary uplift benchmarks used for comparison with the causal forest. The goal is to evaluate whether each benchmark estimates the size and member-level pattern of treatment benefit, not just whether it predicts factual ED risk.
...
Both GLMNet methods underestimate the average true synthetic benefit, but the X-learner is closer on bias, MAE, and RMSE. More importantly, the X-learner has positive Pearson and Spearman correlation with true benefit, while the T-learner has negative correlation in this run. This suggests the GLMNet X-learner is the stronger of the two GLMNet methods for recovering the synthetic treatment-effect pattern. The validation is a benchmark check for the causal forest analysis; it does not establish that causal forest estimates equal the synthetic formula because a causal-forest true-benefit scoring output is not currently generated by the notebook.
...
- [`glmnet_true_benefit_validation_summary.csv`](Outputs/Uplift/Python/glmnet_true_benefit_validation_summary.csv)
- [`glmnet_true_benefit_scored_test_comparison.csv`](Outputs/Uplift/Python/glmnet_true_benefit_scored_test_comparison.csv)
